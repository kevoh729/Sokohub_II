...
function openProduct(prod) { fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'product_view', data: { productName: prod.name } }) }).catch(() => {}); const params = new URLSearchParams(); params.set('name', prod.name); params.set('price', prod.price); params.set('desc', prod.description || ''); params.set('images', (prod.images || []).join(',')); params.set('category', prod.category || 'Services'); if (prod.whatsapp) params.set('whatsapp', prod.whatsapp); window.location.href = 'product.html?' + params.toString(); }
...
function renderProducts() {
    ...
    if (!grid) return;
    if (products.length === 0) { grid.innerHTML = '<p style="text-align:center;padding:40px;color:#888;">No services available right now</p>'; return; }
...
document.addEventListener('DOMContentLoaded', () => { loadProducts(); updateCartCount(); fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'page_view', data: { page: 'services.html' } }) }).catch(() => {}); });
