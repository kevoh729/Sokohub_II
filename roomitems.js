...
async function loadProducts() {
    try {
        const response = await fetch(API_BASE + '/api/products?category=Room+Items');
        if (response.ok) {
            ...
            products = data.map(p => ({
                ...
                whatsapp: p.whatsapp || '',
                category: p.category || 'Room Items'
            }));
            renderProducts();
        }
...
document.addEventListener('DOMContentLoaded', () => { loadProducts(); updateCartCount(); fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'page_view', data: { page: 'roomitems.html' } }) }).catch(() => {}); });
