...
async function loadProducts() {
    try {
        const response = await fetch(API_BASE + '/api/products?category=House+Items');
        if (response.ok) {
            ...
            products = data.map(p => ({
                ...
                whatsapp: p.whatsapp || '',
                category: p.category || 'House Items'
            }));
            renderProducts();
        }
    } catch (e) {
...
function openProduct(prod) {
    ...
    params.set('category', prod.category || 'House Items');
...
function renderProducts() {
    ...
    if (products.length === 0) {
        grid.innerHTML = '<p style="text-align:center;padding:40px;color:#888;">No House Items products available</p>';
        return;
    }
    grid.innerHTML = products.map((p, i) => `
        <div class="product-card" onclick="openProduct(products[${i}])">
            <div class="product-image"><img src="${escapeHtml(p.images[0])}" alt="${escapeHtml(p.name)}" onerror="this.parentElement.innerHTML='📦'"></div>
