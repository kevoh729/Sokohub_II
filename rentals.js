...
async function loadProducts() {
    try {
        const response = await fetch(API_BASE + '/api/products?category=Rentals');
        if (response.ok) {
...
function openProduct(prod) { fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'product_view', data: { productName: prod.name } }) }).catch(() => {}); const params = new URLSearchParams(); params.set('name', prod.name); params.set('price', prod.price); params.set('desc', prod.description || ''); params.set('images', (prod.images || []).join(',')); params.set('category', prod.category || 'Rentals'); if (prod.whatsapp) params.set('whatsapp', prod.whatsapp); window.location.href = 'product.html?' + params.toString(); }
...
function showToast(msg) { const t = document.createElement('div'); t.style.cssText = 'position:fixed;bottom:20px;right:20px;background:#16803a;color:white;padding:12px 20px;border-radius:8px;font-weight:600;z-index:9999'; t.textContent = msg; document.body.appendChild(t); setTimeout(() => t.remove(), 3000); }
...
function renderProducts() {
    ...
    if (!grid) return;
    if (products.length === 0) { grid.innerHTML = '<p style="text-align:center;padding:40px;color:#888;">No Rentals available</p>'; return; }
...
document.addEventListener('DOMContentLoaded', () => { loadProducts(); updateCartCount(); fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'page_view', data: { page: 'rentals.html' } }) }).catch(() => {}); });
