...
async function loadProducts() {
    try {
        const response = await fetch(API_BASE + '/api/products?category=Electronics');
        if (response.ok) {
...
function openProduct(prod) {
    ...
    params.set('category', prod.category || 'Electronics');
...
function renderProducts() {
    ...
    if (products.length === 0) {
        grid.innerHTML = '<p style="text-align:center;padding:40px;color:#888;">No Electronics products available</p>';
        return;
    }
