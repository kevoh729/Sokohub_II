...
async function loadProducts() {
    try {
        const response = await fetch(API_BASE + '/api/products?category=Study+Essentials');
        if (response.ok) {
            ...
            products = data.map(p => ({
                ...
                whatsapp: p.whatsapp || '',
                category: p.category || 'Study Essentials'
            }));
            renderProducts();
        }
...
function renderProducts() {
    ...
    if (!grid) return;
    if (products.length === 0) { grid.innerHTML = '<p style="text-align:center;padding:40px;color:#888;">No Study Essentials products available</p>'; return; }
